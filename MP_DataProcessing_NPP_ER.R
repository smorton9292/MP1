...
#Change Tank_ID based on conditions for all dates after 3/8/23
data <- data %>%
...
#NPP over time for each Tank_ID - check for removing erroneous values
ggplot(npp_data, aes(x = date, y = NPP, color = Tank_ID)) +
  geom_point() +
  labs(title = "NPP Over Time", x = "Date", y = "NPP (% DO)") +
  theme_minimal()

#ER over time for each Tank_ID - check for removing erroneous values
...
#Remove NPP sampling errors that are represented by impossible values
npp_data <- npp_data %>%
  mutate(NPP = ifelse(NPP < 0, NA, NPP))

#Remove ER sampling errors that are represented by impossible values
er_data <- er_data %>%
...
##For A drop row 78 as it is a error in the sensor prob reading##
